                    ...
                    <tr className="text-[11px] font-mono uppercase tracking-wider text-muted-foreground border-b border-border">
                      {view === "results" && (
                        <th className="text-left font-medium pl-4 pr-1 py-2 w-16">
                          {`'${String(data.adpSeason).slice(2)} ADP`}
                        </th>
                      )}
                      <th className={`text-left font-medium py-2 ${view === "draft" ? "pl-4 pr-1" : "px-1"}`}>Player</th>
                  ...
                  <tbody>
                    {data.rows.map((r, i) => (
                      <tr key={`${r.rankLabel}-${r.name}`} className={`border-b border-border/40 ${i % 2 === 1 ? "bg-muted/10" : ""}`}>
                        ...
                        <td className={`py-2 ${view === "draft" ? "pl-4 pr-1" : "px-1"}`}>
                          {r.playerId ? (
                            <Link
                        ...
                        {view === "draft" ? (
                          <td className={`px-2 py-2 text-right font-mono tabular-nums font-bold ${isTopTenFinish(i + 1) ? "text-terminal-green" : "text-foreground"}`}>
                            {r.rankLabel}
                          </td>
                        ) : (
