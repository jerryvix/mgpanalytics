                  ...
                  <thead>
                    <tr className="text-[11px] font-mono uppercase tracking-wider text-muted-foreground border-b border-border">
                      <th className="text-left font-medium pl-4 pr-1 py-2 w-16">
                        {view === "draft" ? "Rk" : `'${String(data.adpSeason).slice(2)} ADP`}
                      </th>
                      ...
                      <th className="text-right font-medium px-2 py-2">
                        {view === "draft" ? `'${String(data.adpSeason).slice(2)} Proj` : "Pick"}
                      </th>
                      <th className="text-right font-medium px-2 py-2">
                  ...
                  <tbody>
                    {data.rows.map((r, i) => (
                      <tr key={`${r.rankLabel}-${r.name}`} className={`border-b border-border/40 ${i % 2 === 1 ? "bg-muted/10" : ""}`}>
                        <td className={`pl-4 pr-1 py-2 font-mono font-bold ${view === "draft" ? "text-muted-foreground" : isTopTenFinish(i + 1) ? "text-terminal-green" : "text-foreground"}`}>
                          {view === "draft" ? (r.overallRank == null ? "-" : `#${r.overallRank}`) : r.rankLabel}
                        </td>
                        ...
                        <td className="px-2 py-2 text-right font-mono tabular-nums">
                          {view === "draft" ? (
                            <span className={`font-bold ${isTopTenFinish(i + 1) ? "text-terminal-green" : "text-foreground"}`}>
                              {r.rankLabel}
                            </span>
                          ) : (
                            <>
