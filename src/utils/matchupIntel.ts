...
export interface ContinuityInput {
  ...
  netPortal: number | null;
  draftDepartures: number | null;
}
...
/**
 * Continuity score for edge comparison: returning PPA share, nudged by net
 * portal movement and dinged for early NFL departures (a drafted starter is
 * a bigger loss than one portal exit — 2x).
 */
export function continuityScore(input: ContinuityInput): number | null {
  ...
  return input.percentPpa + (input.netPortal ?? 0) - 2 * (input.draftDepartures ?? 0);
}
